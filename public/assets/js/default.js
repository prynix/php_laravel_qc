//var hostname="http://localhost/l4-bbad/public/";
var hostname="qc.tintuc.vn/";
function doFileBackup(){
	...
	$.post(hostname+"lib/Ajax.php", { 'function': "filebackup" },
	   function(response){
...
function doDownload(file){
	setElementState("#warning", false, true);
	setElementState("#success", false, true);
	$.post(hostname+"lib/Ajax.php", { 'function': "download", file: file },
		function(response){
...
function doDelete(file, md5){
	...
	$.post(hostname+"lib/Ajax.php", { 'function': "delete", file: file },
		function(response){
		   	if(response.code == "200"){
...
function doRollback(file, md5){
	...
	$.post(hostname+"lib/Ajax.php", { 'function': "rollback", file: file },
		function(response){
		   	if(response.code == "200"){
...
function doDatabaseBackup(database){
	...
	$.post(hostname+"lib/Ajax.php", { 'function': "databasebackup", "database": database },
	   function(response){
		   	if(response.code == "200"){
...
function doDatabaseRollback(file, md5){
	...
	setElementState("#" + md5 + "_rollback_link", false, false);
	setElementState("#" + md5 + "_rollback_load", true, false);
	$.post(hostname+"lib/Ajax.php", { 'function': "databaserollback", file: file },
		function(response){
		   	if(response.code == "200"){
				setElementState("#" + response.data.md5 + "_rollback_load", false, false);
				setElementState("#" + response.data.md5 + "_rollback_link", true, false);
