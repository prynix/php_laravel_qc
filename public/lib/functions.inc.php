...
function PMBP_dump($db,$tables,$data,$drop,$zip,$comment) {
    ...
    if ($con=PMBP_mysql_connect()) {
        ...
        if ($pclzip->error_code==0) {
        	...
        	echo addOutput("pclzip: ".$pclzip->error_string,"red");
...
//        echo addOutput("THIS DATABASE SEEMS TO CONTAIN 'RING CONSTRAINTS'. pMBP DOES NOT SUPPORT THEM. PROBABLY THE FOLLOWING BACKUP IS BROKEN!","red_left");
        return false;
    }
    return $new_tables;
}
...
function PMBP_save_global_conf($global_conf_path="") {
    ...
    if (is_array($CONF)) {
        foreach($CONF as $item=>$conf) {
            ...
            if ($item=="sql_host" && count($CONF['sql_host_s']) ) {
                $file.="\$CONF['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($_SESSION['sql_host_org'])."\");\n";
            } elseif ($item=="sql_user" && count($CONF['sql_host_s'])) {
                $file.="\$CONF['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($_SESSION['sql_user_org'])."\");\n";
            } elseif ($item=="sql_passwd" && count($CONF['sql_host_s'])) {
                $file.="\$CONF['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($_SESSION['sql_passwd_org'])."\");\n";
            } elseif ($item=="sql_db" && count($CONF['sql_host_s'])) {
                $file.="\$CONF['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($_SESSION['sql_db_org'])."\");\n";
            } else {
                // save the current values for all other settings
                $file.="\$CONF['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($conf)."\");\n";
            }
        }
    }
    ...
    foreach($PMBP_SYS_VAR as $item=>$sys_var) {
    	$file.="\$PMBP_SYS_VAR['".$item."']=htmlspecialchars_decode(\"".htmlspecialchars($sys_var)."\");\n";
    }
    ...
    if (!$CONF['ftp_server']) {
        ...
        if (!$login_result=@ftp_login($conn_id,$CONF['ftp_user'],$CONF['ftp_passwd'])) {
            ...
            foreach($files as $filename) {
                ...
                if (!$check) $out.=addOutput(F_FTP_3.": '".$source_file."' -> '".$dest_file."'","red"); 
                    else $out.=addOutput(F_FTP_4." '".$dest_file."'","green");
            }
...
//	 $encoded['body']="--".$boundary.$lb.implode("--".$boundary.$lb,$parts)."--".$boundary."--".$lb.$lb;
    ...
    if (!@mail($CONF['email'],$subject,$encoded['body'],$headers)) {
    	$out.=addOutput(F_MAIL_5,"red");
    } else {
    	$out.=addOutput(F_MAIL_6." ".$CONF['email'],"green");
    }
...
function PMBP_delete_backup_files($files) {
    ...
    foreach($files as $file) if (!@unlink(PMBP_EXPORT_DIR.$file))
    	$out.=addOutput(sprintf(F_DEL_FAILED,$file),"red");
            ...
            if (is_array($remote_files)) {
                ...
                if (is_array($delete_files)) {
                    foreach($delete_files as $filename) {
                        ...
                        if (!$check) $out.=addOutput(sprintf(F_FTP_5,$dest_file),"red"); 
                            else $out.=addOutput(sprintf(F_FTP_6,$dest_file),"green");
                    }
                }
            }
...
function addOutput($string, $class) {
	global $mode;
	if(!isset($mode)) $mode="web";
	$out="";
...
function PMBP_save_FTP_and_Email($files, $packed=false) {
    ...
    if (!$CONF['ftp_server']) {
    	if($CONF['ftp_use']) $out.=addOutput(C_WRONG_FTP,"red");
    	$ftp_connect_failed=true;
    } elseif (!$conn_id=@ftp_connect($CONF['ftp_server'],$CONF['ftp_port'],$PMBP_SYS_VAR['ftp_timeout'])) {
        if($CONF['ftp_use']) $out.=addOutput(F_FTP_1." '".$CONF['ftp_server']."'!","red");
        $ftp_connect_failed=true;
    } elseif (!$login_result=@ftp_login($conn_id,$CONF['ftp_user'],$CONF['ftp_passwd'])) {
    	if($CONF['ftp_use'])  $out.=addOutput(F_FTP_2." '".$CONF['ftp_user']."'","red");
    	$ftp_connect_failed=true;
    }
    ...
    if ($packed) {
        ...
        if(!$ftp_connect_failed) {
            ...
            if ($check) {
            	$out.=addOutput(F_FTP_4." '".$CONF['ftp_path']."/".$filename."'","green");
            } else {
            	$out.=addOutput(F_FTP_3,"red");
            }
        }
    	...
    	if(!$ftp_connect_failed) {        
            ...
            foreach($files as $filepath) {
                if ($filepath=trim($filepath)) {
                    ...
                    if ($check) {
                    	$out.=addOutput(F_FTP_4." '".$CONF['ftp_path']."/".$dest_file."'","green");
                    } else {
                    	$out.=addOutput(F_FTP_3.": '".$source_file."' -> '".$dest_file."'","red");
                    }
                }
            }
    	}
...
//		$buffer .= "user abborted!";
	...
	if(connection_status()==2||connection_status()==3) {
		$buffer.="<br>".addOutput(F_TIMEOUT,"red");
		if(isset($mode) && $mode=="web") {
			$buffer.= "<br><br>\n";
